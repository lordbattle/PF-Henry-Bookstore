...
const saveAllBooksDb = async (req, res) => {
  try {
    ...
    for (let index = 0; index < 100; index = index = index + 50) {
      ...
      const { data: allBooksApi } = await axios(`${API_URL}startIndex=${index}&maxResults=40&q=%20+title&key=AIzaSyDIEQFwVWL0GFcB3moa_sL8euql4UjAm7k`);
      ...
      for (let i = 0; i < allBooksApi.items.length; i++) {
        ...
        const newBook = await Book.findOrCreate({
          ...
          defaults: {
            subtitle: allBooksApi.items[i].volumeInfo.subtitle ? allBooksApi.items[i].volumeInfo.subtitle : 'It does not have subtitles',
            publishedDate: allBooksApi.items[i].volumeInfo.publishedDate,
            publisher: allBooksApi.items[i].volumeInfo.publisher ? allBooksApi.items[i].volumeInfo.publisher : 'It does not have publisher',
            description: allBooksApi.items[i].volumeInfo.description ? allBooksApi.items[i].volumeInfo.description : 'It does not have description',
            pages: allBooksApi.items[i].volumeInfo.pageCount,
            averageRating: allBooksApi.items[i].volumeInfo.averageRating ? allBooksApi.items[i].volumeInfo.averageRating : 2,
            identifier: stringIdentifier(allBooksApi.items[i].volumeInfo.industryIdentifiers),
            bookPic: allBooksApi.items[i].volumeInfo.hasOwnProperty('imageLinks') ? allBooksApi.items[i].volumeInfo.imageLinks.smallThumbnail : "https://previews.123rf.com/images/tackgalichstudio/tackgalichstudio1411/tackgalichstudio141100020/33575659-s%C3%ADmbolo-de-libro-sobre-fondo-gris.jpg",
        ...
        //Se crea los generos
        allBooksApi.items[i].volumeInfo.categories?.forEach(async (categorie) => {
